...
graph TD
    ...
    F --> H[WebSocket Connection]
    E --> H
    
    H --> I{Authenticated?}
    I -->|Yes| J[Send auth.upgrade]
    I -->|No| K[Send Message]
    J --> L[Receive auth.upgraded]
    L --> K
...
await authManager.verifyCode({ email: "user@example.com", code: "123456" });
```
