...
You can also provide your own conversation ID (useful for restoring conversations from storage):

```typescript
const conversationId = await client.createConversation("My Chat", "my-custom-id");
```
...
### Vanilla widget with pre-seeded conversation
...
attachSunnyChat({
  ...
  config: {},
  anonymous: true,
  conversationId: savedId, // Pre-seed with ID from your storage
});
```
