...
The vanilla widget automatically parses and renders doctor profiles and provider search results from message text:
...
Parse message text and render artifacts in your custom UI:
...
function ArtifactRenderer({ artifact }: { artifact: { item_type: string; item_content: unknown } }) {
  if (artifact.item_type === "doctor_profile") {
    return renderDoctorProfile(artifact.item_content as DoctorProfileArtifact);
  }
  if (artifact.item_type === "provider_search_results") {
    return renderProviderSearchResults(artifact.item_content as ProviderSearchResultsArtifact);
  }
  return renderGenericArtifact(artifact.item_content);
}
```
...
### Display doctor profile from message

```typescript
function DoctorProfileFromMessage({ message }: { message: SunnyAgentMessage }) {
  const artifacts = parseEmbeddedArtifacts(message.text);
  const doctorProfile = artifacts.find(a => a.item_type === "doctor_profile");

  if (!doctorProfile) return null;

  const profile = doctorProfile.item_content as DoctorProfileArtifact;
  return (
...
### Parse and render all artifacts in a message

```typescript
function MessageWithArtifacts({ message }: { message: SunnyAgentMessage }) {
  const artifacts = parseEmbeddedArtifacts(message.text);
