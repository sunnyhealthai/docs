...
The Sunny Agents SDK supports multiple authentication modes: SAML/OIDC via Auth0, custom token exchange, and passwordless authentication. With `createSunnyChat`, all authentication configuration is **server-driven** -- you only specify the auth type and the server provides the necessary Auth0 details automatically.
...
When using `createSunnyChat`, auth configuration is fetched via **HTTP** from the [`/sdk/config`](/api-reference/authentication/get-sdk-config) endpoint _before_ any WebSocket connection is established:
...
sequenceDiagram
    ...
    ConfigAPI-->>SDK: SdkAuthConfig (Auth0 domain, clientId, connection, etc.)
    ...
    alt SAML/OIDC
        ...
        Note over SDK: Verification UI rendered on first message
    end

    Note over SDK: WebSocket connects when user sends first message
    SDK->>WebSocket: Connect + auth.upgrade
...
2. **Fetch config**: SDK calls `GET /sdk/config` to retrieve Auth0 settings
3. **Auto-login**: SDK automatically triggers Auth0 authentication popup
4. **First message**: WebSocket connects when the user sends a message
5. **Upgrade auth**: SDK sends `auth.upgrade` message with ID token
6. **Authenticated**: SDK receives `auth.upgraded` response

**Token Exchange:**
1. **Create chat**: Call `createSunnyChat` with `authType: "tokenExchange"`
2. **Fetch config**: SDK calls `GET /sdk/config` to retrieve token exchange settings
3. **First message**: WebSocket connects when the user sends a message
4. **Provide ID token**: Your app provides an ID token via `idTokenProvider`
5. **Exchange token**: SDK exchanges ID token for access token
6. **Upgrade auth**: SDK sends `auth.upgrade` message with access token
7. **Authenticated**: SDK receives `auth.upgraded` response
8. **Refresh**: SDK automatically refreshes token when expired
...
The SDK automatically refreshes tokens when they expire. No action is needed from your app. If your `idTokenProvider` needs to force-refresh, you can do so per provider:
