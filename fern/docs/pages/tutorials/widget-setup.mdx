...
**Option A: Unified API with automatic authentication (Recommended)**
...
**Option B: Low-level widget with manual configuration**
...
<Tip>
  **Anonymous mode only:** The minimal `attachSunnyChat` example above works for anonymous/local development only. It does not send `sdk.session.create` and cannot use server-driven auth. For production or authenticated use, use `createSunnyChat` with `partnerIdentifier`, `publicKey`, and `authType` from [Sunny Central](/partner-setup).
</Tip>
...
For anonymous/local-only conversations (no server persistence or auth):
...
For production or authenticated sessions, use `createSunnyChat` instead (see [Authenticated mode](#authenticated-mode) below).
...
For production with user authentication, use the unified `createSunnyChat` API:
...
**Or use attachSunnyChat with manual token exchange configuration:**
...
### Pre-seeded conversation ID

For anonymous mode, you can pass a `conversationId` to use a specific conversation (e.g., restored from your own storage):
...
attachSunnyChat({
  ...
  config: {},
  anonymous: true,
  conversationId: "your-saved-conversation-id",
});
```
