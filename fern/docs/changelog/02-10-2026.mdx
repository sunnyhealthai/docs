...
The `createSunnyChat` API now uses **server-driven authentication configuration**. Instead of passing Auth0 details (domain, clientId, connection) directly, you provide three required parameters and the server supplies all auth configuration automatically via `sdk.session.create`.
